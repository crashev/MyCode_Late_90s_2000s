...
//	printf("+ WYSLANE: %d\n+ NIEWYSLANE: %d\n",in,out);
	// zapisujemy,sprawdzamy czas
	time2=time(NULL);
	czas=(int)difftime(time2,clients[k].timelast);
//	printf("Roznica czasu: %d ->",czas);
	clients[k].timelast=time(NULL);
	if (czas<MSGTIMEOUT) {
	    printf("[NO-NOTICE]\n");
//	    say(b,"admin!admin@czat.interia.pl NOTICE %s :Wiadomosc nie zostala dostarczona z powodu limitu: 1/%d\r\n",clients[k].nick,MSGTIMEOUT);
	    say(b,"NOTICE %s :Wiadomosc nie zostala dostarczona z powodu limitu: 1/%d\r\n",clients[k].nick,MSGTIMEOUT);
	 out++;return;
	 } else { 
	 //printf("[YES]\n");
	 in++; }
...
// nieznane
	case 0x8a:
	case 0x87:
//	     fprintf(log,"[?] KODY: %x/%x \n",ptr[0],ptr[1]);
//	     for (x=0; x<len; x++) fprintf(log,"%c|",ptr[x]);
//	     fprintf(log,"\n");
	     break;
