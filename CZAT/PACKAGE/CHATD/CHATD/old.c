...
//	    say(nick[ilu].dom,"users on line : \n\n");
        for (b=0;b<=ilu;b++) 
	   {
	      say(nick[ilu].dom,"=addlist:%s:%d\n",nick[b].nick,nick[b].dom);
	   }	
	   memset(buf,0x0,sizeof(buf));
           b=0;
	   while (b<ilu) 
	    { 
		say(nick[b].dom,"=join:%s:%d\n",nick[ilu].nick,nick[ilu].dom);
		b++;
	    }
           debug(outlog,"??? [%s] has joined \n",nick[ilu].nick);
           ilu++;           
		    //    FD_CLR(cli[a],&fds);
       for (b=0;b<kolejka;b++) 
       {
