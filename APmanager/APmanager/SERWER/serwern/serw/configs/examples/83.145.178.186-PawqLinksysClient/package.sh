...
nvram set wl0_ssid=PawqLinksysRouterv3
...
nvram set pawq_qos_up=250
nvram set pawq_qos_if=vlan1
#lan ipaddr
nvram set lan_ifname=vlan0
nvram set lan_ipaddr=192.168.100.1
...
read-ethers
EOF
cat > /etc/init.d/S60fw2 << EOF
iptables -A FORWARD -i eth1 -o vlan0 -j ACCEPT
EOF
chmod +x /etc/init.d/S60fw2
reboot
