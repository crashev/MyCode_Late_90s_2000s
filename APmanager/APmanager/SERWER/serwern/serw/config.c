...
config_make_ethers(char *pcHostIp, char *pcName) 
{
	    ...
	    pgres = pgsql_query(pgconn, "select al.macaddr, al.hostip from access_lists al, access_points ap where ap.id = al.aid AND ap.apip='%s' AND ap.apname='%s' and al.active='1';", pcHostIp, pcName);
...
config_make_additional(char *pcHostIp, char *pcFile, char *pcDir, char *pcName) 
{
	...
	if (config_make_QoS(pcHostIp, pcName)<0)
		return -1;
...
config_make_QoS(char *pcHostIp, char *pcName) 
{
	    ...
	    pgres = pgsql_query(pgconn, "select al.hostip,qos.bw,qos.bw_max,qos.bw_traffic from access_lists al, access_points ap, queues qos where ap.id = al.aid AND qos.id = al.qid AND ap.apip='%s' AND ap.apname='%s' AND al.active='1';", pcHostIp, pcName);
	    if (!pgres) {
		fclose(fQoS);
		pgsql_zakoncz(pgconn);
	        return -1;	
